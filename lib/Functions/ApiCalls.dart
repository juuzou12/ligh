...
class ApiCalls {
  ...
  void loginApiCall(
      String email, String password, onSuccess, onWrongCred, onNotFound) async {
  ...
  void changeUserInfo(
      ...
      String bio,
      String phoneNumber,
      String userCollectionDocId,
      onSuccess,
      onFailure) async {
    /*printing out the data sent to firebase*/
    print(
        "---username:[$username]---imageUrl:[$imageUrl]---bio:[$bio]---phoneNo:[$phoneNumber]---");
  ...
  void signUpApiCall(
      String email, String password, onSuccess, onWrongCred, onExists) async {
    try {
      UserCredential userCredential = await FirebaseAuth.instance
  ...
  void addUserDetails(
    ...
    print(
        "---imageUrl;[$imageUrl]--username:[$username]--follower:[$followers]--");
    /*this is the format at which the documentId will be generated*/

    final String userCollectionDocId;
    userCollectionDocId =
        '${_firebaseAuth.currentUser.uid}-${DateTime.now().microsecondsSinceEpoch}';
  ...
  void imageUpload(File imageFile, String imageUrl, onSuccess, onFailed) async {
    ...
    try {
      StorageUploadTask uploadTask = storageReference
          .child('/user_images')
          .child(
              'userId:${_firebaseAuth.currentUser.uid}-${DateTime.now().microsecondsSinceEpoch}')
          .putFile(imageFile);
  ...
  void phoneNumberVerification(String phoneNumber, String code, onSuccess,
      onFailed, onCodeSent, onRetrieval) async {
  ...
  void uploadCollection(
      String title, String comment, List<String> itemIDs, onSuccess, onFailed) {
    /*collection includes a group of posts/ stories/ storeItems*/
    print(
        "---title:[$title]---comment:[$comment]---listOfItemIDs:[$itemIDs]--");
  ...
  void uploadMedia(String episodeTitle, String episodePlot, String videoUrl) {
    ...
    print(
        "---episodeTitle:[$episodeTitle]----episodePlot:[$episodePlot]----videoUrl:[$videoUrl]");
  }

  /*creating a new media series*/
  void createNewSeries(String seriesTitle, String description, String category,
      String seriesCover) {
    /*print out the data been sent to the firebase*/
    print(
        "---seriesTitle:[$seriesTitle]----description:[$description]----category:[$category]-----seriesCover:[$seriesCover]");
    final String userCollectionDocId;
    userCollectionDocId =
        '${_firebaseAuth.currentUser.uid}-${DateTime.now().microsecondsSinceEpoch}';
    fireStoreInstance
  ...
  void uploadSeriesCover(File seriesImageFile, String seriesCover) {
    /*print out the data been sent to the firebase*/
    print(
        "---seriesImageFile:[$seriesImageFile]----seriesCover:[$seriesCover]");
  }
}
